...
from .utils.io import load_image_rgb, load_driving_info, load_driving_paths, resize_to_limit
    ...
    def execute(self, args: ArgumentConfig):
        ...
        if is_video(args.driving_info):
            ...
            if osp.isdir(args.driving_info):
                driving_paths = load_driving_paths(args.driving_info)
            else:
                driving_paths = [osp.join(osp.dirname(args.driving_info), "{:06d}.jpg".format(i)) for i in range(n_frames) ]
        ...
        if inference_cfg.flag_pasteback:
            ...
            if args.output_image:
                for i, img in enumerate(I_p_paste_lst):
                    save_path = osp.join(args.output_dir, f'{basename(args.source_image)}--{basename(driving_paths[i])}.jpg')
            ...
            if args.output_image:
                for i, img in enumerate(I_p_lst):
                    save_path = osp.join(args.output_dir, f'{basename(args.source_image)}--{basename(driving_paths[i])}.jpg')
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                    cv2.imwrite(save_path, img)
